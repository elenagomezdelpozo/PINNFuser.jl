...
tsteps = range(5.0, 5.0 + numb , length = num_of_samples)
...
for (k, (u, t)) in enumerate(zip(solved_ode.u, solved_ode.t))
...
pinn_problem = ODEProblem(pinn_ode!, ode_problem.u0, tspan) # what if here we put just u0 instead of ode_problem.u0
solved_pinn = solve(
    pinn_problem,
    Vern7(),
    saveat = range(tspan[1], tspan[2], length = num_of_samples),
...
print(sum(du_mat[:,1]))
print(du_nn_mat[1000,1]-du_nn_mat[2000,1])
print(sum(du_mat[1000:2000, 1])*(1//1000))
print(sum(du_nn_mat[1000:2000, 1])*(1//1000))
