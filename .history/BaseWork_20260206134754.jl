...
tspan = (0.0, 1.0)
num_of_cycles = 4 # 1 cycle for training
...
function zero_mean_loss_correct(f_mat, nn_vars)
    ...
    for i in nn_vars
        for j in 1:cycles
            ...
            for k in 1:(150-1)
                integral[i, j] += (f_mat[k+1 + (j-1)*150, i] - f_mat[k + (j-1)*150, i])
                # simplified: 0.5*(f[k+1]-f[k] + f[k+1]-f[k]) / dt * dt = f[nt]-f[1]
            end
        end
    end
    return integral
...
pinn_zm_loss = zero_mean_loss_correct(pinn_to_plot, [1,2,3,4]) 
print(pinn_to_plot[1, 1], pinn_to_plot[150,1], pinn_to_plot[300, 1], pinn_to_plot[450, 1], pinn_to_plot[600, 1])


# -----------USING THE TRAINED NN-------------
num_of_cycles = 10 # 1 cycle for training
