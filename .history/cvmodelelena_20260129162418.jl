...
function Valve(R, deltaP, open)
    dq = 0.0
    if (open) > 0.0
        dq = (deltaP / R) * (1 / (1 + exp(-50.0 * deltaP)))
    else
        dq = 0
    end
    return dq
...
"""
function Valve(R, deltaP, open_signal)
    # 1. Smoothly determine if the valve is 'active' based on the signal
    # We use a sigmoid to replace the 'if (open) > 0.0'
    activation = 1 / (1 + exp(-10.0 * open_signal))
    
    # 2. Smoothly determine flow based on pressure difference
    # You already had this part, which is good!
    flow_strength = (deltaP / R) * (1 / (1 + exp(-50.0 * deltaP)))
    
    # The result is now differentiable everywhere
    return activation * flow_strength
end
