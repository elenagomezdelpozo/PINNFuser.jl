...
original_problem = ODEProblem(pinn_ode!, ode_problem.u0, tspan)
solved_pinn = solve(
    pinn_problem,
    Vern7(),
    saveat = range(tspan[1], tspan[2], length = num_of_samples),
...
us_phys = hcat(solved_phys.u...)'

plots = [
    plot(ts, us_phys[:, i], label="physics", lw=2)
    |> p -> plot!(p, ts, us[:, i], label="physics + NN", lw=2, ls=:dash)
    |> p -> plot!(p, xlabel="time", ylabel=labels[i])
    for i in 1:7
]
