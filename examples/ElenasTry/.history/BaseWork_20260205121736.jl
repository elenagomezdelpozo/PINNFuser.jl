...
tsteps = range(5.0, 5.0 + num_of_cycles * τ , length = num_of_samples)
loaded_data = readdlm("/Applications/Desktop/CODE/Thesis/PINNFuser.jl/examples/OneChamberModelCVS/original_data.txt")
extrap_original_data = Array{Float64}(loaded_data)[1:Int(floor(tspan[2] * num_of_samples_per_cycle)), :]
original_data = extrap_original_data[751:750 + num_of_samples, :]
new_tseps = range(extrapolation_tspan[1], tspan[2], length = Int(floor(extrapolation_tspan[2] * 150)))
...
pinn_problem = ODEProblem(pinn_ode!, u0, extrapolation_tspan) 
solved_pinn = solve(
    pinn_problem,
    Vern7(),
    saveat = new-tseps,
...
#--------PLOT-----------
t_data_full = range(0, 10, length = size(extrap_original_data, 1))
