...
u0 = [7.0, 7.0, 7.0, 200.0, 0.0, 0.0, 0.0] # Initial conditions pLV, psa, psv, Vlv, Qav, Qmv, Qs
pinn_problem = ODEProblem(pinn_ode!(du, u0, p, t), u0, tspan) 
solved_pinn = solve(
    pinn_problem,
    Vern7(),
    saveat = new_tseps,
...
# number of states
n_states = length(solved_pinn.u[1])
n_time   = length(solved_pinn.u)

# stack states into matrix (time × state)
U = hcat(solved_pinn.u...)'   # same as pinn_pred

# compute NN corrections at each time step
NNcorr = zeros(n_time, n_states)
...
for i in nn_vars
    p = scatter(
        U[:, i], NNcorr[:, i],
        xlabel = state_names[i],
        ylabel = "NN correction to du[$i]",
        title  = "NN correction vs $(state_names[i])",
        markersize = 3,
        legend = false
    )
    push!(plots, p)
...
plot(plots..., layout = (length(plots), 1), size = (700, 250 * length(plots)))
