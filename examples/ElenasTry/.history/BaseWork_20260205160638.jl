...
function NIK_plus!(du, u, p, t)
    pLV, psa, psv, Vlv, Qav, Qmv, Qs = u

    du[1] =
        (Qmv - Qav) * ShiElastance(t, Eₘᵢₙ, Eₘₐₓ, τ, τₑₛ, τₑₚ, Eshift) +
        pLV / ShiElastance(t, Eₘᵢₙ, Eₘₐₓ, τ, τₑₛ, τₑₚ, Eshift) *
        DShiElastance(t, Eₘᵢₙ, Eₘₐₓ, τ, τₑₛ, τₑₚ, Eshift) +0.1
    # Left Ventricle
    du[2] = (Qav - Qs) / Csa + 0.1 #Systemic arteries
    du[3] = (Qs - Qmv) / Csv + 0.1 # Venous
    du[4] = Qmv - Qav # LV volume
    du[5] = Valve(Zao, (du[1] - du[2]), u[1] - u[2])  # AV 
    du[6] = Valve(Rmv, (du[3] - du[1]), u[3] - u[1])  # MV
    du[7] = (du[2] - du[3]) / Rs # Systemic flow
...
ode_problem = ODEProblem(NIK_plus!, u0, tspan, params)
...
time_to_plot = new_tseps[751:1500]
...
t_data_full = range(5, 10, length = 750)
