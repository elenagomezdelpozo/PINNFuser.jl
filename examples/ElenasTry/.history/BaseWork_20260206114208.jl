...
function zero_mean_loss(du_mat, t, nn_vars) 
    integral = zeros(length(nn_vars)) for (j, i) in enumerate(nn_vars) for k in 1:length(t)-1 dt = t[k+1] - t[k] integral[j] += 0.5 * dt * (du_mat[k, i] + du_mat[k+1, i]) end end return 1e6 * mean(abs2, integral) end
