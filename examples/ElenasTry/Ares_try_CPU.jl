...
data = load("Trainings/trained_pinn_model_CPU.jld2")
...
p2 = plot(epochs, losses,
    xlabel = "Epoch",
    ylabel = "Loss",
    title = "Training Loss over Epochs for CPU",
    lw = 2,
    marker = :circle,
    markersize = 3,
    legend = false)

savefig(p2, "Figures/CPU_loss.png")
...
for j in 1:nn_vars
    ...
    plot!(p3[j], epochs, var_history,
          title = labelss[j],
          xlabel = "Epochs",
          ylabel = "NN Contribution CPU",
          legend = true)
end
savefig(p3, "Figures/CPU_nn.png")