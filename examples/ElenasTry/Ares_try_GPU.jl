...
data = load("Trainings/trained_pinn_model_GPU.jld2")
...
p2 = plot(epochs, losses,
    xlabel = "Epoch",
    ylabel = "Loss",
    title = "Training Loss over Epochs for GPU",
    lw = 2,
    marker = :circle,
    markersize = 3,
    legend = false)

savefig(p2, "Figures/GPU_loss.png")
...
p3 = plot(layout = (2,3), size=(1200,800))  # 2 rows x 3 columns for 6 variables

for j in 1:nn_vars
    # Extract history of variable j across epochs
    var_history = [nn_history[i,1][j] for i in 1:n_epochs]
    
    plot!(p3[j], epochs, var_history,
          title = labelss[j],
          xlabel = "Epochs",
          ylabel = "NN Contribution GPU",
          legend = true)
end
savefig(p3, "Figures/GPU_nn.png")